 ...
 *  @author Kevin Wayne
 */
public final class StdAudio88k {

    /**
     *  The sample rate - 44,100 Hz for CD quality audio.
     */
    public static int SAMPLE_RATE = 88200;
    ...
    private StdAudio88k(int samplingRate) {
    	SAMPLE_RATE = samplingRate;
    	init();
    }
    
    public static void init(int samplingRate) {
    	SAMPLE_RATE = samplingRate;
    	init();
    }
    ...
    private static byte[] readByte(String filename) {
        ...
        try {
            ...
            // try to read from URL
                ...
                URL url = StdAudio88k.class.getResource(filename);
    ...
    public static synchronized void play(final String filename) {
        ...
        InputStream is = StdAudio88k.class.getResourceAsStream(filename);
        if (is == null) {
    ...
    ***************************************************************************/
    ...
    private static double[] note(double hz, double duration, double amplitude) {
        int n = (int) (StdAudio88k.SAMPLE_RATE * duration);
        double[] a = new double[n+1];
        for (int i = 0; i <= n; i++)
            a[i] = amplitude * Math.sin(2 * Math.PI * i * hz / StdAudio88k.SAMPLE_RATE);
        return a;
    }
    ...
    public static void main(String[] args) {
        ...
        // 440 Hz for 1 sec
        double freq = 440.0;
        for (int i = 0; i <= StdAudio88k.SAMPLE_RATE; i++) {
            StdAudio88k.play(0.5 * Math.sin(2*Math.PI * freq * i / StdAudio88k.SAMPLE_RATE));
        }
        ...
        for (int i = 0; i < steps.length; i++) {
            ...
            StdAudio88k.play(note(hz, 1.0, 0.5));
        }
